...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    if temperature < 800 and neutrons_emitted > 500 and product < 500000:
        return True  # Reactor is critical
    else:
        return False  # Reactor is not critical
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    threshold_percentage = 0.90 * threshold  
    criticality_percentage = 0.10 * threshold

    # Calculate the range for within 10% above or below the threshold
    lower_limit = threshold - criticality_percentage
    upper_limit = threshold + criticality_percentage
    
    if output < threshold_percentage:
        status = 'LOW'
    elif lower_limit <= output <= upper_limit:
        status = 'NORMAL'
    else:
        status = 'DANGER'
